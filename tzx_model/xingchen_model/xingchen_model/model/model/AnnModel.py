...
def ann_model(nb_feature, nb_class):
    '''
    ...
    model = Sequential()
    # 添加输入层150
    model.add(Dense(36, activation='relu', input_shape=(nb_feature,)))
    # 添加隐藏层
    ...
    optimizer = RMSprop(lr=0.005, decay=1e-6)
...
def run(data_path=class_4_36_path, columns_name=column_4_36_data, train_test_rate=0.3, drop_outliers=True,
    ...
    # 定义模型并打印出模型结构
    model = ann_model(nb_feature, nb_class)
    print(model.summary())
    # 创建一个history实例
    # 训练模型
